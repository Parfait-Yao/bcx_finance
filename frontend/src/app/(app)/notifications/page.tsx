...
import { Bell, BellRing, Sparkles, X, Loader2 } from 'lucide-react';
...
export default function NotificationsPage() {
  const { data: notifications, chargement, horsLigne, recharger } = useOfflineCache<NotificationItem[]>('/notifications', 'notifications');
    ...
    <main className="max-w-2xl mx-auto px-4 pt-6 space-y-4">
      ...
      {chargement && !notifications ? (
        ...
        <ul className="space-y-2">
          {notifications.map((n) => (
            <li key={n.id} onClick={() => !horsLigne && !n.lu && marquerCommeLue(n.id)}
              ...
              {n.lu ? <Bell size={20} className="text-gray-400 mt-0.5 shrink-0" />
                : n.type === 'ia_conseil' ? <Sparkles size={20} className="text-primaire mt-0.5 shrink-0" />
                : <BellRing size={20} className="text-accent mt-0.5 shrink-0" />}
              <div>
                <p className="text-xs font-semibold text-primaire mb-1">{LIBELLES_TYPE[n.type] || n.type}</p>
                <p className="text-sm">{n.message}</p>
                <p className="text-xs text-gray-400 mt-1">{formaterDate(n.createdAt)}</p>
              </div>
              {!horsLigne && (
                <button onClick={(e) => supprimer(e, n.id)} aria-label="Supprimer"
